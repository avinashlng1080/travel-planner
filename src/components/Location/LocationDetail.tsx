...
export default function LocationDetail() {
  ...
  if (!selectedLocation) {
    return (
      <div className="h-full flex items-center justify-center bg-white text-slate-600 p-8">
  ...
  return (
    <div className="h-full bg-white text-slate-900 overflow-y-auto">
      <div className="sticky top-0 bg-white border-b border-slate-200 p-4 flex items-start justify-between z-10">
        <div className="flex-1">
          ...
          <p className="text-slate-600 text-sm mt-1">{selectedLocation.city}</p>
        </div>
        <button
      ...
      <div className="p-4 space-y-6">
        ...
        {selectedLocation.address && (
          <div className="flex items-start gap-2">
            ...
            <div>
              <h3 className="font-semibold mb-1 text-sm text-slate-600 uppercase tracking-wide">Address</h3>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-lg p-3">
            ...
            <p className="text-slate-900 font-medium">{selectedLocation.estimatedDuration}</p>
          </div>
          <div className="bg-white rounded-lg p-3">
            <div className="flex items-center gap-2 text-slate-600 text-xs mb-1">
            ...
            <p className="text-slate-900 font-medium">{selectedLocation.grabEstimate}</p>
          </div>
          <div className="bg-white rounded-lg p-3">
            <div className="flex items-center gap-2 text-slate-600 text-xs mb-1">
            ...
            <p className="text-slate-900 font-medium">{selectedLocation.distanceFromBase}</p>
          </div>
          <div className="bg-white rounded-lg p-3">
            <div className="flex items-center gap-2 text-slate-600 text-xs mb-1">
              <Clock className="w-4 h-4" />
            ...
            <p className="text-slate-900 font-medium">{selectedLocation.drivingTime}</p>
          </div>
        </div>

        <div className="bg-white rounded-lg p-4">
          <h3 className="font-semibold mb-2 text-sm text-slate-600 uppercase tracking-wide">Opening Hours</h3>
          <p className="text-slate-600">{selectedLocation.openingHours}</p>
        </div>

        {selectedLocation.entranceFee && (
          <div className="bg-white rounded-lg p-4">
            <h3 className="font-semibold mb-2 text-sm text-slate-600 uppercase tracking-wide">Entrance Fee</h3>
            <p className="text-slate-600">{selectedLocation.entranceFee}</p>
          </div>
        )}
        ...
        {selectedLocation.dressCode && (
          <div className="bg-amber-900/30 border border-amber-700 rounded-lg p-4">
            ...
            <p className="text-slate-600">{selectedLocation.dressCode}</p>
          </div>
        )}

        {selectedLocation.tips && selectedLocation.tips.length > 0 && (
          <div>
            <h3 className="font-semibold mb-3 text-sm text-slate-600 uppercase tracking-wide flex items-center gap-2">
        ...
        {selectedLocation.whatToBring && selectedLocation.whatToBring.length > 0 && (
          <div>
            <h3 className="font-semibold mb-2 text-sm text-slate-600 uppercase tracking-wide">What to Bring</h3>
        ...
        <div>
          <h3 className="font-semibold mb-2 text-sm text-slate-600 uppercase tracking-wide">Best Time to Visit</h3>
          <div className="flex flex-wrap gap-2">
            {selectedLocation.bestTimeToVisit.map((time, index) => (
              <span key={index} className="bg-white text-slate-600 px-3 py-1 rounded-full text-sm">
                {time}
              </span>
            ))}
          </div>
