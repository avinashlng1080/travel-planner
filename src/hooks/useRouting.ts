...
export function useRouting(waypoints: RoutePoint[], enabled = true): RoutingResult {
  ...
  const cacheRef = useRef<Map<string, { coordinates: RoutePoint[]; distance?: number; duration?: number }>>(new Map());
  ...
  useEffect(() => {
    ...
    const fetchRoute = async () => {
      ...
      try {
        ...
        if (!apiKey) {
          console.warn('[useRouting] No OpenRouteService API key found. Using straight line fallback.');
        ...
        if (import.meta.env.DEV) {
          console.log(`[useRouting] Route: ${routeDistance.toFixed(1)}km, ${routeDuration.toFixed(0)}min`);
        }
      } catch (err) {
